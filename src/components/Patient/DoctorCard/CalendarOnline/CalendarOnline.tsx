...
export default function CalendarOnline(props: Props) {
  ...
  const handleNavigateCheckout = (hour: string) => {
    ...
    navigate(
      `/checkout?date=${appointment_date.toLocaleString()}&online=${true}&expertID=${
        props.expert?._id
      }`
    );
  };
            ...
            {calHours(selectedDay) !== undefined
              ? calHours(selectedDay).map((hour: any, index: number) => {
                  return (
                    ...
                    >
                      <h1
                        className="text-sm font-bold text-color-dark-primary lg:text-base"
                        key={index}
                      >
                        {hour}
                      </h1>
