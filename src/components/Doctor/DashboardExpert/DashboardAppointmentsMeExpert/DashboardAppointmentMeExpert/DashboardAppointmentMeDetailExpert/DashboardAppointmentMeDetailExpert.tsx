...
export default function DashboardAppointmentMeDetailExpert({}: Props) {
  const { id } = useParams();
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const authExpertDownloadProfilePictureResponse =
        await fetchExpertProfilePicture(appointment?.appointment_owner?._id);
      setProfileImageLoader(true);
        ...
        <div className="flex flex-col items-start justify-start gap-5">
          <div className="flex items-start justify-start gap-5">
            ...
            <div className="flex flex-col items-start justify-start">
              <Link to={`/experts/${appointment?.appointment_owner?._id}`}>
                <div className="flex flex-col items-start justify-start">
                  <h5 className="text-sm font-bold opacity-50">
                    {appointment?.appointment_owner?.expert_title !== undefined
                      ? appointment?.appointment_owner.expert_title.title_title
                      : ""}
                  </h5>
                  <div className="flex items-center justify-center gap-1">
                    <h5 className="text-sm">
                      {appointment?.appointment_owner?.expert_name}
                    </h5>
                    <h5 className="text-sm">
                      {appointment?.appointment_owner?.expert_surname}
                    </h5>
                  </div>
                </div>
              </Link>
              <div className="flex items-center justify-center gap-1 text-sm">
                ...
                <a
                  href={`mailto:${appointment?.appointment_owner?.expert_email}`}
                >
                  {appointment?.appointment_owner?.expert_email}
                </a>
              </div>
            </div>
          </div>
